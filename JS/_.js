...
try {
    ...
    function parseInput(input) {
        ...
        return {
            axioms: lines.slice(0, -1).map(line => line.split(/[~<]?=+[>]?/g).map(s => s.match(/\S+/g))),
            proofStatement: lines[lines.length - 1]
        };
    } // end parseInput
    
    function generateProof(axioms, proofStatement) {
        let [lhs, rhs] = proofStatement.split(/[~<]?=+[>]?/g).map(s => s.match(/\S+/g));
        let steps = [];
        ...
        return `${proofFound ? 'Proof' : 'Partial-proof'} found!\n\n${proofStatement}, (root)\n` +
            steps.map ((step,i,me) => {
                    ...
                    return `${step.side === 'lhs' ? step.result.join (' ') : step.other.join (' ')} = ${step.side === 'lhs' ? step.other.join (' ') : step.result.join (' ')}, (${step.side} ${step.action}) via ${step.axiom}`;
                }).join ('\n') 
                    + (proofFound ? '\n\nQ.E.D.' : '');
    } // end generateProof
